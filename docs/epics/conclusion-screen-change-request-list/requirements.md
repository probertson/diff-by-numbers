---
title: "[TUI] List Comments from the conclusion screen"
id: "#65"
---
## Background

When a Reviewer advances past the last Step of a Walkthrough, the TUI shows the conclusion screen:
a one-line summary of how many Comments they raised, and the prompt to Hand Off. It is the
last stop before handing the round back to the Authoring Agent, and the natural moment to look over
what they raised. Today they cannot do that from there: the key that opens the Comment list
does nothing on the conclusion screen, so checking their Comments means stepping back into
...
- The conclusion screen shows "You raised N Comments across M Steps." followed by "Press h
  ...
  [6].
- The Comment list is a full screen, not an overlay. From it the Reviewer can edit a Comment
  or withdraw one (after a confirmation) [7]. A Step can also open a filtered form of it,
  showing only the Comments on one line [8].
...
- The list refreshes after every save or delete, and shows an empty-state message once nothing is
  left in it [11].
- The Reviewer-facing term is "Comment". #34 renamed it from "Change Request" across every
  surface [12].
...
- [8] `internal/tui/tui.go:660` — `e` on a line with more than one Comment
...
1. As a Reviewer on the conclusion screen who has raised at least one Comment, I see the
   line "Press l to see your Comments." between the summary line and the Hand Off line,
   separated from each by a blank line like the existing lines.
   1. The noun agrees with the count: "Comment" when I have raised exactly one,
      "Comments" otherwise.
   2. When I have raised none, the line does not appear.
2. As a Reviewer on the conclusion screen, I can press `l` to open the list of all my Comments,
   unfiltered, whatever their count.
...
4. As a Reviewer in the list, I can view, edit and withdraw Comments the same way whether I
...
5. As a Reviewer leaving the list, I return to where I opened it: the conclusion screen if I opened
   ...
   1. On returning to the conclusion screen, its summary and the line in requirement 1 reflect any
      edits and withdrawals I made; if I withdrew my last Comment, the line is gone.
6. As a Reviewer in the edit screen that I reached from the list, when I save, cancel or delete, I
   ...
   2. Editing a Comment directly on a Step, without going through the list, still returns
      me to the Step.
7. As a Reviewer, I can check, edit and withdraw my Comments from the conclusion screen and
...
1. **After an edit that began in the list, return to the list** (requirement 6). Rejected:
   ...
   drop the Reviewer on the last Step, a detour out of the "check my Comments, then Hand
   Off" flow.
...
4. **With no Comments raised, hide the prompt line but keep the list key working and in the
...
5. **Word the new line with "Comments"** (requirement 1). The ask said "Change Requests"; #34 has
   since renamed the term on every surface, and this line follows it. Rejected: keeping the ask's
   wording, which would leave this one line on the old term.
6. **The list shows no status message after an edit or delete.** Rejected: showing the Step view's
   status messages ("Comment updated", "could not update the Comment", and so on) in
   the list, because the refreshed list already shows the current state: an edited note shows its
   new text or its old one, and a deleted Comment is gone.

## Out of scope

- Re-raising a declined Comment from the conclusion screen: the ask covers only the list,
